@tool
class_name XRToolsInteractableHingeYAxis
...
##
## The hinge rotates itself around its local Y axis, and so should be
...
## If true, the hinge moves to the default position when released
...
func _do_move_hinge(position: float) -> float:
	...
	# Move if necessary
	if position != _hinge_position_rad:
		transform.basis = Basis.from_euler(Vector3(0, position, 0))
